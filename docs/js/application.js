...
var Actions = {
  updateBpm: function updateBpm(bpm) {
    store.dispatch({
      type: "updateBpm",
      payload: {
        bpm: bpm
  ...
  updateDisplayBpm: function updateDisplayBpm(bpm) {
    store.dispatch({
...
var App = function () {
  function App() {
    ...
    store.subscribe(function () {
      _this3.clickScheduler.bpm = store.getState().bpm;
    });

    var light = new Light(document.querySelector("#light"));
    this.lightScheduler = new LightScheduler(light);

    var tenkey = new TenKey(document.querySelector("#tenkey"), { value: store.getState().displayBpm });
    store.subscribe(function () {
      tenkey.value = store.getState().displayBpm;
    });

    store.subscribe(function () {
      var s = store.getState().displayBpm.toString();
      while (s.length < 3) {
        s = "0" + s;
      }
